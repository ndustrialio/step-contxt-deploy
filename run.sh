...
RESULT=$(curl -d "payload=$json" -H "Authorization: $WERCKER_CONTXT_DEPLOY_TOKEN" -s "http://coordinator.api.ndustrial.io/v1/services/$WERCKER_CONTXT_DEPLOY_SERVICE_DESCRIPTOR/deploy" --output "$WERCKER_STEP_TEMP"/result.txt -w "%{http_code}")
